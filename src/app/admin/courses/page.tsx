...
import {
  ...
  restoreCourseAction,
  deleteCourseAction,
  bulkTrashCoursesAction,
  bulkRestoreCoursesAction,
  bulkDeleteCoursesPermanentlyAction,
...
export default function AdminCoursesPage() {
  ...
  const trashCourses = courses.filter((c) => c.isDeleted)
  ...
  const handleBulkTrash = async () => {
    if (!confirm(`Move ${selectedIds.length} courses to Trash?`)) return
    const res = await bulkTrashCoursesAction(selectedIds)
    if (res.success) {
      setSelectedIds([])
      setStatusMsg({ type: 'success', text: `${selectedIds.length} courses moved to Trash` })
      fetchData()
    } else {
      setStatusMsg({ type: 'error', text: res.error || 'Failed to bulk trash courses' })
    }
  }

  const handleBulkRestore = async () => {
    const res = await bulkRestoreCoursesAction(selectedIds)
    if (res.success) {
      setSelectedIds([])
      setStatusMsg({ type: 'success', text: `${selectedIds.length} courses restored` })
      fetchData()
    } else {
      setStatusMsg({ type: 'error', text: res.error || 'Failed to bulk restore courses' })
    }
  }

  const handleBulkDeletePermanently = async () => {
    const res = await bulkDeleteCoursesPermanentlyAction(selectedIds)
    if (res.success) {
      setSelectedIds([])
      setBulkConfirmModal(false)
      setStatusMsg({ type: 'success', text: `${selectedIds.length} courses permanently deleted` })
      fetchData()
    } else {
      setBulkConfirmModal(false)
      setStatusMsg({ type: 'error', text: res.error || 'Failed to delete courses' })
    }
  }
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-semibold text-slate-700">
                {selectedIds.length > 0
                  ? `${selectedIds.length} of ${filteredCourses.length} selected`
              ...
              {filteredCourses.map((course) => {
                ...
                <tr
                  ...
                  className={`hover:bg-slate-50 transition-colors ${
                    selectedIds.includes(course.id) ? 'bg-blue-50/50' : ''
                  }`}
                >
                  <td className="p-4" onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      onChange={() => toggleSelectId(course.id)}
                      className="w-4 h-4 rounded cursor-pointer"
                    />
                  </td>
