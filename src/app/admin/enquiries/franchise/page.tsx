...
export default function AdminFranchiseProposalsPage() {
  ...
  const handleBulkStatusChange = async (newStatus: string) => {
    ...
    startTransition(async () => {
      const res = await bulkUpdateEnquiryStatusAction('franchise', selectedIds, newStatus)
      if (res.success) {
        setStatusMsg({ type: 'success', text: res.message || `Selected proposals marked as ${newStatus}` })
        setProposals((prev) =>
          prev.map((item) =>
            selectedIds.includes(item.id) ? { ...item, status: newStatus } : item
          )
        )
        if (selectedProposal && selectedIds.includes(selectedProposal.id)) {
          setSelectedProposal((prev: any) => ({ ...prev, status: newStatus }))
        }
        setSelectedIds([])
      } else {
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === filteredProposals.length) {
      ...
      setSelectedIds(filteredProposals.map((r) => r.id))
    }
  }
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="relative flex-1 w-full">
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 placeholder-slate-400 font-medium focus:outline-none focus:border-indigo-600 focus:bg-white transition-all"
          />
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <select
            ...
            className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 font-semibold focus:outline-none focus:border-indigo-600 w-full sm:w-auto"
          >
        ...
        <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl shadow-xs flex flex-wrap items-center justify-between gap-3 animate-fadeIn">
          <div className="flex items-center gap-3">
            <span className="bg-white px-3 py-1 rounded-lg border border-slate-200 text-xs font-semibold text-slate-800 flex items-center gap-1.5 shadow-2xs">
              ...
              {selectedIds.length} of {filteredProposals.length} Selected
            </span>
            <button
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              disabled={isPending}
              className="px-3 py-1.5 bg-white hover:bg-indigo-50 text-indigo-700 border border-slate-200 hover:border-indigo-300 text-xs font-medium rounded-lg transition-colors flex items-center gap-1.5 shadow-2xs disabled:opacity-50"
            >
              <Icon icon="ion:call-outline" className="w-3.5 h-3.5 text-indigo-600" />
              Mark Contacted
            </button>
                ...
                <tr>
                  <th className="p-3.5 w-10">
                    <input
                      ...
                      title={selectedIds.length === filteredProposals.length ? 'Deselect all' : 'Select all'}
                    />
                  </th>
                ...
                {filteredProposals.map((p) => (
                  <tr
                    ...
                    className={`hover:bg-indigo-50/50 cursor-pointer transition-colors group ${
                      selectedIds.includes(p.id) ? 'bg-indigo-50/30' : ''
                    }`}
                  >
